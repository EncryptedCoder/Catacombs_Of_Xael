...
Mummy::Mummy(SDL_Renderer& renderer, std::string& spriteSheet, std::string& mName, char &symbol, float &level, float &health, int &endurance, int &money, float& yPos, float& xPos, int &zPos, int &bonusHeal) : Enemy(renderer, spriteSheet, symbol, mName, level, health, endurance, money, yPos, xPos, zPos, bonusHeal)
{
	mummyTexture = TextureManager::LoadTexture(renderer, spriteSheet);
}


Mummy::~Mummy()
{
	SDL_DestroyTexture(mummyTexture);
}


//How does the enemy move
bool Mummy::move(Player* player, Zone* zone, GameManager* GM)
{
   ...
   //will eventually add range in place of 1...
	...
	{//if the enemy is close we print the details
		...
		if (hitChance < 40) //enemy % chance to hit you...
		{
			doesHit = true;
			this->moveY = 0;
			this->moveX = 0;
...
{
	SDL_RenderCopy(&renderer, mummyTexture, &srcRect, &destRect);
}