...
Skeleton::Skeleton(SDL_Renderer& renderer, std::string& spriteSheet, std::string& sName, char &symbol, float &level, float &health, int &endurance, int &money, float& yPos, float& xPos, int &zPos, int &bonusHeal) : Enemy(renderer, spriteSheet, symbol, sName, level, health, endurance, money, yPos, xPos, zPos, bonusHeal)
{
	skeletonTexture = TextureManager::LoadTexture(renderer, spriteSheet);
}
...
bool Skeleton::move(Player *player, Zone *zone, GameManager *GM)
{
   ...
   //will eventually add range in place of 1...
	...
	{//if the enemy is close we print the details
		...
		if (hitChance < 18) //enemy % chance to hit you...
		{
			doesHit = true;
			this->moveY = 0;
			this->moveX = 0;
...
void Skeleton::update(Zone* zone)
{
	...
	//int mapWidth = zone->MapHEIGHTSy[this->getZpos()];
...
{
	SDL_RenderCopy(&renderer, skeletonTexture, &srcRect, &destRect);
}